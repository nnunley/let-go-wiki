...
sources:
  ...
  - "repo: nooga/let-go cmd/lgbgen/main.go @ 83931029 (bundle-skip mechanism; ir.data bootstrap moved after the bundle write by #838), cmd/lgbstat on pkg/rt/core_compiled.lgb @ a9c183f9 and 83931029, 2026-09-10"
created: "2026-07-02"
updated: "2026-09-10"
status: stable
---
...
- `data.lg` was the exception to that ordering until #838 (merged 2026-09-10, `83931029`). `lgbgen` bootstrapped it into the shared const pool first so downstream namespaces could resolve `ir/*` symbols, and that pool is what the bundle serialises, so its compiled functions rode along: `cmd/lgbstat` at `a9c183f9` attributed 111 of 937 chunks to `<embedded:ir.data:lgbgen-bootstrap>`. The bootstrap now runs inside `compileIRForLowering`, after the bundle is written, which took `core_compiled.lgb` from 311,302 to 273,748 bytes and boot with it.
