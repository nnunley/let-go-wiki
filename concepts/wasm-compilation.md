...
sources: ["repo: nooga/let-go wasm/, docs/guide/usage.md, pkg/bundle, 2026-07-02", "repo: nooga/let-go pkg/cli/cli.go, docs/guide/usage.md @ 638b4a6a, 2026-09-07", "repo: nooga/let-go pkg/cli/cli.go, pkg/rt/core/ir/passes/entry_frame.lg, docs/guide/usage.md @ 36b13f79, 2026-09-20", "issue: nooga/let-go#796 closed by pr #902, in v1.13.0, 2026-09-20"]
created: "2026-07-02"
updated: "2026-09-20"
status: stable
---
...
An AOT native-entry binary keeps that promise through a compile-time omission. `lg-compile --entry-frame` emits a frame that replays the program's top-level forms, through `rt.LoadProgramNamespaces` and then `rt.RunProgramMainChunk`, before it calls the entry, and `*compiling-aot*` is false throughout. The guard `docs/guide/usage.md` recommends for exactly this situation, `(when-not *compiling-aot* (-main))`, therefore passed during the replay, and the binary entered the program twice: once on the VM, then once natively. #902 closed that (#796) in v1.13.0 by building the frame's bytecode with `lg -c -entry-frame-entry ns/-main`, which omits only the selected top-level entry call. Every other top-level form, including guarded initialization in required namespaces, still runs, and still sees the var false. Which half of the prologue runs the top level depends on the bundle. `lg -c` emits an empty namespace table only for a single-file program; once the program requires another namespace, `MainChunk` is one of the `NSOrder` chunks, so the top level runs during the namespace load and the main-chunk replay returns early by design.
