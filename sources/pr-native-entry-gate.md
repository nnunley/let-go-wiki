...
sources: ["pr: https://github.com/nooga/let-go/pull/729, 2026-09-05", "repo: nooga/let-go pkg/rt/run.go, pkg/rt/core/ir/passes/entry_frame.lg @ 8f65bf68, 2026-09-19", "issue: nooga/let-go#796; pr #902 (open), 2026-09-19"]
created: "2026-09-05"
updated: "2026-09-19"
status: active
---
...
- **Follow-up:** #796 rides on the replay this PR added. Those top-level forms include the guard `docs/guide/usage.md` recommends, `(when-not *compiling-aot* (-main))`, and the var is false at runtime, so an `--entry-frame` binary built from a guarded program enters the entry once on the VM and once natively. The empty-NS-table framing above is single-file only: once a program requires another namespace, `MainChunk` is one of the `NSOrder` chunks, the top level runs inside `LoadProgramNamespaces`, and `RunProgramMainChunk` no-ops — so both halves of the prologue can be the one that runs the guard. #902 is open against it.
